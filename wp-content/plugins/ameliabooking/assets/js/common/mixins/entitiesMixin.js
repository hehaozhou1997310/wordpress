      ...
      entities.employees.forEach((employee) => {
        employee.serviceList.forEach((service) => {
          Object.assign(
            ...
            {price: service.price, minCapacity: service.minCapacity, maxCapacity: service.maxCapacity}
          )
        })
      })
    ...
    fetchEntities (callback, options) {
      ...
      if (options.isPanel || (this.$root.hasApiCall && !this.entitiesLoaded())) {
        this.$http.get(`${this.$root.getAjaxUrl}/entities`, config).then(response => {
          ...
          window.ameliaAppointmentEntities = JSON.parse(JSON.stringify(response.data.data))

          this.fillCachedEntities(window.ameliaAppointmentEntities)

          this.processEntities(window.ameliaAppointmentEntities)

          if (this.$root.useTranslations) {
            this.translateEntities(window.ameliaAppointmentEntities)
          }
