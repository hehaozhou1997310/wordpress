...
if(!class_exists('cLR_Plugin_Settings'))
{
	class CLR_Plugin_Settings
	{
        ...
        function localize_plugin(){
            // register styles
            wp_register_style('clr_plugin_css', CLR_URL . 'css/clr-style.css', null, CLR_VERSION);
        ...
        function clr_login_shortcode(){
            if (is_user_logged_in()) {
                ...
                $logout_redirect = (empty($wp_redirect_settings['wp_logout_redirect']) || $wp_redirect_settings['wp_logout_redirect'] == '-1') ? '' : $wp_redirect_settings['wp_logout_redirect'];                
                echo 'Logged in as <strong>' . ucfirst($current_user->user_login) . '<>. <a href="' . wp_logout_url(get_permalink($logout_redirect)) . '">Log out ? </a>';
                return;
            }  
        ...
        {   
            ...
            $userdata = array(
                            ...
                            'user_country'  => esc_attr($this->contry),
                            'user_phone'   => esc_attr($this->phone),
