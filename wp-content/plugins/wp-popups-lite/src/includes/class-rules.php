		...
		if ( $popups ) {
			foreach ( $popups as $pop ) {
				...
				foreach ( $rules as $group_id => $group ) {
					...
					// all rules must have matched in group, so we can show popup
					if ( $match_group ) {
				...
				foreach ( $rules as $group_id => $group ) {
					...
					if ( is_array( $group ) ) {
						foreach ( $group as $rule_id => $rule ) {
							...
							if ( ! in_array( $rule['rule'], $rules_to_check ) ) {
								// Mark the popup as having other rules, so we know it needs further proccesing with ajax
								$popups[ $i ]->need_ajax = true;
								continue;
							}
							$match = apply_filters( 'wppopups_rules_rule_match_' . $rule['rule'], $rule );
							// We examined popups, meaning that if we don't have filtered ones it's because they were excluded.
							$examined ++;
							if ( ! $match ) {
								$match_group = false;
								break;
