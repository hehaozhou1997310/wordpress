;/* global wppopups_settings,*/(function($) {
	...
	var WPPopups = {
		...
		ready: function() {
            ...
            // init popups that doesn't need ajax
            $(".spu-box").each(function() {
                const $popup 	= $(this),
                    $need_ajax		= $popup.data('need_ajax');
                if( $need_ajax ) {
        ...
        triggerByClass : function( value, $popup ) {
            ...
            // Only add the trigger if not cookies are set for popup
        ...
        formSubmissions: function ($popup) {
            ...
            $(document).on('gform_post_render', function(jqXHR, form_id){
                ...
                const $form = $('#gform_ajax_frame_' + form_id);
                if( ! $form.is('.spu-box *') ) {
                    return;
                }
                const contents = $form.contents().find('*').html(),
         ...
         * @param $popup
         */
        formOptinSubmissions: function ($popup) {
            const popup_id = $popup.data('id'),
            ...
            data.push({ name : 'nonce' , value : wppopups_vars.nonce });
            data.push({ name : 'id' , value : popup_id });
            const ajax       = {
                url:      wppopups_vars.ajax_url,
                data:     data,
                cache:    false,
        ...
        loadValidation: function() {
            ...
            if ( typeof $.fn.validate !== 'undefined' ) {
                ...
                $.validator.addMethod( 'check-limit', function( value, element ) {
                    ...
                    let choiceLimit = parseInt( $( element ).closest( 'ul' ).attr( 'data-choice-limit' ) || 0, 10 );
                ...
                $( '.wppopups-validate' ).each( function() {
                    ...
                    // TODO: cleanup this BC with wppopups_validate.
         ...
         * @param disable
         */
        toggleButton: function ( $submit_btn, disable ) {

            const oldText = $submit_btn.data( 'submit-text' ),
        ...
        checkBindActions: function(target) {
            ...
            for (let i = 0; i < $href.length; i++) {
                if( $href[i] && target.attr('href') == $href[i] ) {
                    return true;
                }
            }
